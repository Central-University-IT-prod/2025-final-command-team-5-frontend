        ...
        try {
          ...
          setQrCode(
            typeof response === 'string'
              ? response
              : 'https://prod-team-5-qnkvbg7c.REDACTED/checkQr/' +
  ...
  const submitTicket = async () => {
    ...
    try {
      const finalDescription =
        ticketData.ticketType === 'OTHER' ? ticketData.description : selectedIssue;

      const token = localStorage.getItem('userToken');

      const response = await axios.post(
        'https://prod-team-5-qnkvbg7c.REDACTED/api/tickets/create',
        {
  ...
  useEffect(() => {
    switch (ticketData.ticketType) {
      ...
      case 'TECHNICAL_PROBLEM':
        ...
        break;
      case 'PLACE_TAKEN':
        setPredefinedDescriptions(['Место занято']);
        break;
      case 'FOOD':
        setPredefinedDescriptions([
          'Не хватает еды',
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-all duration-200">
      <div className="p-4">
        ...
        {item.status === 'PENDING' ||
        (new Date(item.startAt) <= new Date() && new Date() <= new Date(item.endAt)) ? (
          <div className="flex">
            <button
      ...
      >
        <div
          className={`bg-white p-4 rounded-lg shadow-sm mb-4 flex justify-center transition-all duration-300 ease-in-out 
                ${
        ...
        <div
          className={`flex flex-col sm:flex-row gap-2 transition-all duration-300 ease-in-out
                ${
                  showDetails ? 'opacity-100 transform scale-100' : 'opacity-0 transform scale-95'
                }`}
          style={{
        ...
        <ModalPortal>
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 transition-all duration-300 ease-in-out">
            ...
            >
              ...
              <p className="text-gray-600">Предъявите этот QR-код при входе</p>
            </div>
          </div>
        </ModalPortal>
      )}
        ...
        <ModalPortal>
          <div className="fixed inset-0 z-50 flex items-center justify-center transition-all duration-300 ease-in-out">
            <div
              ...
              style={{
                backdropFilter:
                  rescheduleModalMounted && !isRescheduleModalClosing ? 'blur(5px)' : 'blur(0px)',
                backgroundColor:
                  rescheduleModalMounted && !isRescheduleModalClosing
                  ...
                  <div className="mt-6 p-3 rounded-lg bg-yellow-50 border border-yellow-200">
                    <div className="flex items-start">
                      ...
                      <p className="text-sm text-yellow-700">
                        Место бронирования останется прежним:{' '}
                ...
                <div className="flex justify-end space-x-3 pt-4 border-t">
                  ...
                  >
                    <div className="relative flex items-center justify-center">
                      ...
                      <div
                        className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ${
                          !reschedulingLoading && rescheduleStatus === 'success'
                            ? 'opacity-100'
                            : 'opacity-0'
                        }`}
                      >
                      ...
                      <div
                        className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ${
                          !reschedulingLoading && rescheduleStatus === 'error'
                            ? 'opacity-100'
                            : 'opacity-0'
                        }`}
                      >
        ...
        <ModalPortal>
          <div className="fixed inset-0 z-50 flex items-center justify-center transition-all duration-300 ease-in-out">
            <div
              ...
              style={{
                backdropFilter:
                  ticketModalMounted && !isTicketModalClosing ? 'blur(5px)' : 'blur(0px)',
                backgroundColor:
                  ticketModalMounted && !isTicketModalClosing
                    ? 'rgba(255, 255, 255, 0.3)'
                    : 'rgba(255, 255, 255, 0)',
              }}
                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto">
                      {predefinedDescriptions.map((desc, index) => (
                        ...
                        >
                          ...
                          <span
                            className={`text-sm ${
                              selectedIssue === desc ? 'text-blue-700 font-medium' : 'text-gray-700'
                            }`}
                          >
                            {desc}
                          </span>
                ...
                <div className="flex justify-end space-x-3 pt-4 border-t">
                  ...
                  >
                    <div className="relative flex items-center justify-center">
                      <span
                        className={`transition-all duration-300 ${
                          ticketSubmitting || ticketStatus ? 'opacity-0' : 'opacity-100'
                        }`}
                      >
                        Отправить
                      </span>
                      ...
                      <div
                        className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ${
                          ticketSubmitting ? 'opacity-100' : 'opacity-0'
                        }`}
                      >
                        <svg
                          className="animate-spin h-5 w-5 mr-2"
                      ...
                      <div
                        className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ${
                          !ticketSubmitting && ticketStatus === 'success'
                            ? 'opacity-100'
                            : 'opacity-0'
                        }`}
                      >
                      ...
                      <div
                        className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ${
                          !ticketSubmitting && ticketStatus === 'error'
                            ? 'opacity-100'
                            : 'opacity-0'
                        }`}
                      >
        ...
        <ModalPortal>
          <div className="fixed inset-0 z-50 flex items-center justify-center transition-all duration-300 ease-in-out">
            <div
              ...
              style={{
                backdropFilter:
                  cancelModalMounted && !isCancelModalClosing ? 'blur(5px)' : 'blur(0px)',
                backgroundColor:
                  cancelModalMounted && !isCancelModalClosing
                    ? 'rgba(255, 255, 255, 0.3)'
                    : 'rgba(255, 255, 255, 0)',
              }}
